...
Solution CBS::find_solution(Map &map, const Task &task, const Config &cfg)
{
    ...
    do
    {
		...
		if (config.use_edge_split && info.first.add_node!=-1 && conflict.move1.id1!=conflict.move1.id2){
			//cout<<"move1: ";
			//prt_move(conflict.move1);
			Move temp_move=modify_move(conflict.move1,info.first.add_node);
			//cout<<"new_m: ";
			//prt_move(temp_move);
		...
		if (config.use_edge_split && info.second.add_node!=-1 && conflict.move2.id1!=conflict.move2.id2){
			//cout<<"move2: ";
			//prt_move(conflict.move2);
			Move temp_move=modify_move(conflict.move2,info.second.add_node);
			//cout<<"new_m: ";
			//prt_move(temp_move);
		...
		*/
        ...
        CBS_Node right({pathA}, parent, constraintA,info.first, node.cost + pathA.cost - get_cost(node, conflict.agent1), 0, node.total_cons + 1);
        CBS_Node left ({pathB}, parent, constraintB,info.second, node.cost + pathB.cost - get_cost(node, conflict.agent2), 0, node.total_cons + 1);
...
bool CBS::validNewNode(Vector2D X1,Vector2D X2,Vector2D New){
	//check if new node is still in the way of original path
	// return true if valid
	Vector2D line1(New-X1),line2(New-X2),lineb(X1-X2);
	double side1(sqrt(line1*line1)),side2(sqrt(line2*line2)),base(sqrt(lineb*lineb));
	double s((side1+side2+base)/2);
	double A(sqrt(s*(s-side1)*(s-side2)*(s-base)));//heron's formula
	double h=2*A/base;
...
CBS::Map_delta_pair CBS::split_edge(Conflict conflict, std::vector<sPath> paths){
	...
	if (node11==node12){
		...
		if (t>-config.precision){
			New=Vector2D(map->fit2grid(i0+v.i*t),map->fit2grid(j0+v.j*t)); 
			int new_idB(map->add_node(New.i,New.j, node21, node22,conflict.agent2));
			if (new_idB!=-1){
				retval.second=Map_delta(new_idB,{node21,node22}); 
				h_values.add_node(new_idB,conflict.agent2,node22);
		...
		if (t>-config.precision){
			New=Vector2D(map->fit2grid(i0+v.i*t),map->fit2grid(j0+v.j*t)); 
			int new_idA(map->add_node(New.i,New.j, node11, node12,conflict.agent1));
			if (new_idA!=-1) {
				retval.first=Map_delta(new_idA,{node11,node12}); 
				h_values.add_node(new_idA,conflict.agent1,node12);
				//cout<<" &&&:"<<new_idA<<"@("<<New.i<<","<<New.j<<") a:"<<conflict.agent1;
			}
		}
...
void CBS::saveCT(const string &fileName, CBS_Node *goal_node, unsigned int agent_num) { // write the CT to a file
...
void CBS::gen_original_map(CBS_Node *node){
	CBS_Node* curNode = node;
	while (curNode->parent != nullptr){
		map->alter_back(curNode->delta);
		curNode = curNode->parent;
	}
}

void CBS::prt_map_delta_pair(Map_delta_pair delta_pair){
